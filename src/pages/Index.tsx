...
const Index = () => {
  ...
  return (
    <div className="w-full max-w-[393px] h-screen mx-auto bg-gradient-to-br from-pink-50 via-purple-50 to-yellow-50 flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-16 left-4 w-7 h-7 bg-pink-200 rounded-full animate-pulse opacity-30"></div>
      <div className="absolute top-32 right-6 w-5 h-5 bg-yellow-200 rounded-full animate-bounce opacity-40"></div>
      <div className="absolute bottom-32 left-6 w-4 h-4 bg-purple-200 rounded-full animate-pulse opacity-35"></div>
      ...
      {/* Bottom Navigation */}
      <div className="bg-white/90 backdrop-blur-lg border-t-2 border-pink-100 px-5 py-4 pb-safe-bottom relative">
        {/* Cute top decoration */}
        <div className="absolute -top-1 left-1/2 transform -translate-x-1/2 w-14 h-1 bg-pink-200 rounded-full"></div>
        
        <nav className="flex justify-around items-center">
          <button
            onClick={() => setActiveTab("scan")}
            className={`flex flex-col items-center space-y-2 px-5 py-3 rounded-xl transition-all duration-300 ${
              activeTab === "scan"
